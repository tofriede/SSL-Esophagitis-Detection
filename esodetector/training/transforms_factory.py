...
from timm.data.transforms import str_to_pil_interp, RandomResizedCropAndInterpolation, \
    ResizeKeepRatio, CenterCropOrPad, RandomCropOrPad, MaybeToTensor, MaybePILToTensor
from timm.data.naflex_transforms import RandomResizedCropToSequence, Patchify
...
def transforms_imagenet_train(
    ...
    if rotation:
        primary_tfl += [transforms.RandomApply([
            transforms.Lambda(lambda x: transforms.functional.rotate(
                x, angle=random.choice([0, 90, 180, 270])
            ))
        ], p=1.0)]
