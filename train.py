...
from esodetector.utils.eval import get_class1_metrics, calculate_auc_roc_scores, calculate_auc_pr_scores
from esodetector.training.dataset_factory import create_train_dataset
from esodetector.training.loader import create_loader as create_training_loader
...
parser = argparse.ArgumentParser(description='PyTorch Esophagitis Training')
...
group.add_argument('--data-dir', metavar='DIR', default=DATA_DIR,
...
group.add_argument('--class-map', default=CLASS_MAP, type=str, metavar='FILENAME',
...
group.add_argument('--repeat-class', default=None, type=int, nargs='+',
                   help='Class index(es) to repeat in the training dataset (e.g., --repeat-class 0 or --repeat-class 0 1)')
group.add_argument('--repeat-factor', default=1, type=int,
                   help='How many times to repeat samples from the specified class(es) (default: 1, no repetition)')
...
group.add_argument('--rotation', type=bool, default=True,
                   help='Apply 0, 90, 180, 270 degree random rotation augmentation.')
...
group.add_argument('--seed', type=int, nargs='+', default=[42], metavar='S',
                   help='random seed(s) (default: 42). Can provide multiple seeds for multiple runs.')
...
def _run_training(
    ...
    if utils.is_primary(args):
        _logger.info(f'Using class map: {args.class_map}')
    
    dataset_train = create_train_dataset(
        args.dataset,
        root=args.data_dir,
    ...
    # Use standard loader
    loader_train = create_training_loader(
        dataset_train,
    ...
    if utils.is_primary(args):
        ...
        # If multiple seeds, create seeds subfolder
        if multi_seed_mode:
            base_output = args.output if args.output else './output/train'
            output_dir = utils.get_outdir(os.path.join(base_output, exp_name, 'seeds'), f'seed_{seed}')
        else:
    ...
    # Clean up GPU memory after each seed iteration to prevent OOM in multi-seed runs
    if multi_seed_mode:
        # Delete large objects that hold GPU memory
        del model
...
def train_one_epoch(
    ...
    amp_autocast: Callable = suppress,
...
def validate(
    ...
    # Collect all predictions and targets for F1 score and AUC calculation
    all_predictions = []
    all_targets = []
    ...
    with torch.inference_mode():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            # Get predictions and probabilities for F1 score and AUC calculation
    ...
    if utils.is_primary(args):
        log_name = 'Test' + log_suffix
        _logger.info(
            f'{log_name} AUC Scores:  '
            f'AUC_PR: {auc_pr:.4f}  '
            f'AUC_ROC: {auc_roc:.4f}  '
        )
        _logger.info(
            f'{log_name} Class 1 Metrics:  '
            f'Sensitivity/Recall: {sensitivity_class1:.4f}  '
            f'Specificity: {specificity_class1:.4f}  '
            f'Precision: {precision_class1:.4f}'
        )
        _logger.info(
            f'{log_name} F1 Scores:  '
            f'Macro: {f1_macro:.4f}  '
        )
